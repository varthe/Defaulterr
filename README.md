...
- **partial_run_cron_expression**: Specify a cron expression (e.g., `0 3 * * *` for daily at 3:00 AM) to do a partial run on a schedule.See [https://crontab.guru/](https://crontab.guru/). It is recommended to only use this or the Tautulli webhook, not both simultaneously.
